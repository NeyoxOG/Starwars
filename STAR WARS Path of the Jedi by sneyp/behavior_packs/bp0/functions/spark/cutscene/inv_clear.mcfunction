### function to clear inventory of all players
### called: manually

replaceitem entity @a slot.hotbar 0 minecraft:air
replaceitem entity @a slot.hotbar 1 minecraft:air  
replaceitem entity @a slot.hotbar 2 minecraft:air 
replaceitem entity @a slot.hotbar 3 minecraft:air
replaceitem entity @a slot.hotbar 4 minecraft:air
replaceitem entity @a slot.hotbar 5 minecraft:air  
replaceitem entity @a slot.hotbar 6 minecraft:air 
replaceitem entity @a slot.hotbar 7 minecraft:air
replaceitem entity @a slot.hotbar 8 minecraft:air